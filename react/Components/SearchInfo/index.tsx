...
const SearchInfo = () => {
  ...
  const subCategories = data?.categories[0]?.children
    ?.map((categorie: any) => {
      return categorie?.children
    })
    .flat()

  const facetsSelected = selectedFacets?.filter(
    (facet: any) => facet?.key === 'c'
  )
  ...
  useEffect(() => {
    if (page === 'store.search#category') {
      setDataToShow(
        categories?.filter((categorie: any) => {
          return (
  ...
  return (
    <div className={`${handles['search-info__container']}`}>
      ...
      <p className={`${handles['search-info__content']}`}>
        {dataToShow?.metaTagDescription}
      </p>
      <ul className={`${handles['search-info__bullets--container']}`}>
        {dataToShow?.children?.map((chil: any, i: number) => {
          return (
            <li
              key={`dataToShow_${i}`}
              className={`${handles['search-info__bullet']}`}
            >
