...
const MobileMenu = (navItems: any) => {
  ...
  useEffect(() => {
    const subMenuSelected = navItems?.navItems?.find(
      (element: any) => element.id === categorySelected.categoryNumber
    )
  ...
  useEffect(() => {
    const subMenu2Selected = subMenuToShow?.menuItems?.find(
      (element: any) => element.id === subCategorySelected.subCategoryNumber
    )
    ...
    <section className={`w-100 ${handles['mobile__menu-container']}`}>
      ...
      {categorySelected.isSomeCategorySelected && (
        <SubMenu
          ...
          name={subMenuToShow?.name}
          href={subMenuToShow?.href}
          handleShowSub={handleShowSub}
          back={back}
          setBack={setBack}
          highlightTitle={subMenuToShow?.highlightTitle}
        />
      )}

      {subCategorySelected.isSomeSubCategorySelected && (
        <SubMenu2
          subMenu2ToShow={subMenu2ToShow}
          name={subMenu2ToShow?.name}
          href={subMenu2ToShow?.href}
          back2={back2}
          setBack2={setBack2}
        />
      )}
